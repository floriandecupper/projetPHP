...
function beforeroute(){
 ...
 function home(){
  ...
  foreach($array as $key=>$nbr) {
    ...
    $annonces=$App->mget('pu_annonce','etat=?', array(0));
  }
  ...
  function tags() {
    ...
    $tags=$App->mget('pu_tags','id>?', array(0), array('order','tag ASC'));
    $json_data=array();
  ...
  function contact() {
    ...
    if(F3::get('VERB')=='POST')
    {
        ...
        }else
        {
            ...
            $contenu = "<p>Bonjour ".F3::get('POST.pseudo').",<br /><br />Bonjour,<br />Nous avons bien receptionné votre message et nous nous efforcerons d'y répondre dans les plus brefs délais.<br /><br />Cordialement,<br />L'équipe de ".F3::get('site_nom');
